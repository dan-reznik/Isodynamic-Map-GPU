...
So in the above, the (red) equilateral is sent to the green triangle, which is sent to the blue triangle, and then back to the red equilateral, forever if the map continued on! For M sliding within an identity zone, the green an blue triangles move change, but a third application of the map invariably goes back to the original.
